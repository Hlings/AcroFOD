...
# command: python cityscapes_to_yolo.py
def convert_annotation(image_id, paths): # for every image-id  convert its annotation
    global label_map

    def find_box(points):  # find xmin, xmax, ymin ,ymax to generate box
        _x, _y = [float(pot[0]) for pot in points], [float(pot[1]) for pot in points]
        return min(_x), max(_x), min(_y), max(_y)

    def convert(size, bbox):  # convert xyxy to xywh
        # size: (width， length)
    ...
    # output to ：final_output_path / f'{image_id}_leftImg8bit.txt'
    ...
    for obj in load_dict['objects']:  
        ...
        # yolo-style annotation：img.jpg -> img.txt
        output_cache.append(f'{label_map[obj_label]} {x} {y} {w} {h}\n')
    
    shutil_img_flag = 0
    if len(output_cache) != 0: # only caring 'car' class   
...
if __name__ == '__main__':
    ...
    # prepare the map for each classes and the corresponding mark  eg. {'car': 0, 'person': 1}
    label_map = { 'person':0, 'rider':1, 'car':2, 'truck':3, 'bus':4, 'train':5, 'motorcycle':6, 'bicycle':7 }
        ...
        for cities_name in os.listdir(image_dir / _t_):
            ...
            for img_id in image_ids:
                ...
                if img_flag:
                    ...
                    shutil.copy(_final_img_path / img_file, image_output_root_dir / _t_ / img_file) #f'images/{_t_}/{img_file}') 
